...
**Human/AI split (opt-in, hidden by default).** The Human Online / AI Autonomous
split (how much of the active time the user was interacting vs. the AI running
on its own) is hidden by default. Only if the user's request explicitly asks for
it — e.g. "human online", "AI autonomous", "人机拆分", "人在线", "AI 自主",
"展开 Human/AI" — append `--show-human-ai`:
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/time-report/scripts/time-report.py <project> <range> --show-human-ai --open
```
With the flag, the terminal prints the Human online / AI autonomous lines and the
Human / AI-solo columns of the threshold table, and the HTML report's two stat
cards plus the human-interaction gap histogram start expanded. Without it they
are collapsed, but the HTML always has a "▸ 展开 Human/AI 拆分" toggle so the
user can expand them in the page at any time.

Human Online counts every recorded human action, not just typed prompts: short
confirmations, Esc interrupts, **AskUserQuestion answers**, prompts queued while
the AI was working, slash commands, and plan approvals. Claude Code records the
last four outside the `user`/`assistant` message stream (or, for
AskUserQuestion, disguised as an ordinary `tool_result`), so each needs its own
detection rule. `references/HUMAN_ACTIONS.md` classifies **every** record type
seen in real sessions as human-intervention vs. AI-autonomous, lists the three
human actions that are unrecoverable from the logs (tool-permission approvals,
model switches, permission-mode switches — no per-event timestamp exists), and
records the measured biases. Read it before changing any human-signal logic.
